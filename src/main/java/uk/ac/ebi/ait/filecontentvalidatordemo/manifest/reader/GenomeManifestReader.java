...
import uk.ac.ebi.ait.filecontentvalidatordemo.manifest.processor.ASCIIFileNameProcessor;
import uk.ac.ebi.ait.filecontentvalidatordemo.manifest.processor.FileSuffixProcessor;
...
//					.meta().optional().name( Field.ANALYSIS_REF     ).desc( Description.ANALYSIS_REF     ).processor( factory.getAnalysisProcessor() ).and()
					...
					.file().optional().name( Field.CHROMOSOME_LIST  ).desc( Description.CHROMOSOME_LIST  ).processor( getChromosomeListProcessors() )
